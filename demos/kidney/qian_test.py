...
output_dir = f'{parent}/experiment_files/kidney_experiment_01'
...
with open(f'{parent}/experiment_files/best_path.txt', "r") as f:
    for line in f:
        ...
        if len(values) == 6:
            ...
            rot = np.array([values[3], values[4], values[5]])
            contact_points.append(point)
            rotations.append(rot)

num_sim = len(contact_points)

for i in range(num_sim):
    num_transducers = 1
    ray_number = 32
    simulation_dir = f'{output_dir}/pose_{i}'
    ...
    pt, normal = test_transducer_set.place_on_mesh_voxel(0, surface_mesh,contact_points[i], voxel_size[0])
    ...
    about_nl_axis = geometry.Transform(rotation=tuple(rotations[i]), translation=(0, 0, 0), about_axis=True)
    pose = geometry.Transform.make_from_heading_vector(normal, pt)
    ...
    subfolder = f"{output_dir}/output_files"
    ...
    image = image_matrices[:ray_number, :, :]
...
# Write pose to CSV
# TODO: test pose save!!!!!!
    csv_filename = f"{subfolder}/poses.csv"
    with open(csv_filename, "a", newline='') as f:
        writer = csv.writer(f)
        # writer.writerow(["filename", "x", "y", "z", "qx", "qy", "qz", "qw"])  # header
        for _, pose in zip(range(num_transducers), test_transducer_set.transmit_poses()):
            T = pose.get()
            x, y, z = T[:3, 3]
